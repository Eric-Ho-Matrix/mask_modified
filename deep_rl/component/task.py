...
class DynamicGrid(BaseTask):
    def __init__(self, name, env_config_path=None, log_dir=None, seed=None, max_steps=100):
        ...
        change_matrix = np.array([[0, 0, 0],  # base task
                                  # change goal location (reward function) only
                                  [1, 0, 0],
                                  [0, 1, 0],  # change transition function only
                                  [0, 0, 1],  # change input distribution only
                                  [1, 1, 0],  # change reward fn and transition fn
                                  # change reward fn and input distribution
                                  [1, 0, 1],
                                  # change transition fn and input distribution
                                  [0, 1, 1],
        ...
        if self.one_hot_labels:
            for idx in range(len(self.tasks)):
                ...
                self.tasks[idx]['name'] = 'dynamic_grid_task_{0}'.format(
                    idx + 1)
        else:
            labels = np.random.uniform(
                low=-1., high=1., size=(len(self.tasks), self.task_label_dim))
    ...
    def random_tasks(self, num_tasks, requires_task_label=True):
        tasks_idx = np.random.randint(
            low=0, high=len(self.tasks), size=(num_tasks,))
        if requires_task_label:
...
class DynamicGridFlatObs(DynamicGrid):
    ...
    def __init__(self, name, env_config_path=None, log_dir=None, seed=None, max_steps=100):
        super(DynamicGridFlatObs, self).__init__(
            name, env_config_path, log_dir, seed, max_steps)
...
class CTgraph(BaseTask):
    ...
    def step(self, action):
        ...
        if done:
            state = self.reset()
        if self.env_config['image_dataset']['1D']:
            state = state.ravel()
        return state, reward, done, info
    ...
    def random_tasks(self, num_tasks, requires_task_label=True):
        ...
        if requires_task_label:
            ...
            # randomly sampled labels from uniform distribution
            tasks_label = np.random.uniform(
                low=-1., high=1., size=(len(all_tasks), self.task_label_dim))
            tasks_label = tasks_label.astype(np.float32)
            tasks = []
            for idx in tasks_idx:
                task = all_tasks[idx]
...
class MetaCTgraph(BaseTask):
    ...
    def __init__(self, name, env_config_path, log_dir=None):
        ...
        for config_path in env_meta_config['config_paths']:
            env = gym.make(
                'CTgraph-v0', config_path='{0}/{1}'.format(base_path, config_path))
            if env.oneD:
...
class MetaCTgraphFlatObs(MetaCTgraph):
    def __init__(self, name, env_config_path, log_dir=None):
        super(MetaCTgraphFlatObs, self).__init__(
            name, env_config_path, log_dir)
        # overwrite previous written statedim to be flat 1d vector observations
        self.state_dim = int(np.prod(self.env.observation_space.shape))
...
class ContinualWorld(BaseTask):
    ...
    RANDOMIZATION_STRATEGIES = [
        ...
        # randomly generate a subtask per reset in env/task.
        'random_init_all',
        # randomly selected out of 20 predefined subtask per reset in env/task
        'random_init_fixed20',
    ]
    ...
    def __init__(self, name, env_config_path, log_dir=None, seed=1000):
        ...
        self.observation_space = self.envs[env_names[0]].spec.observation_dim
        self.action_space = self.envs[env_names[0]].env.action_space  # (4,)
        ...
        # print(f"[ContiniueWorld init] self.actionspace = {self.action_space}")
        ...
        self.tasks = [{'name': env_names[i], 'task': env_names[i],
                       'task_label': None} for i in range(env_config['label_dim'])]
        self.task_label_dim = env_config['label_dim']
        print(
            f"[task] task_label_dim = {self.task_label_dim}, len tasks = {len(self.tasks)}")
        # generate label for each task
        if self.one_hot_labels:
            for idx in range(len(self.tasks)):
                label = np.zeros((self.task_label_dim,)).astype(np.float32)
...
class ProcessTask:
    ...
    def random_tasks(self, num_tasks, requires_task_label):
        self.pipe.send([ProcessWrapper.RANDOM_TASKS, [
                       num_tasks, requires_task_label]])
        return self.pipe.recv()
...
class ParallelizedTask:
    def __init__(self, task_fn, num_workers, log_dir=None, single_process=False):
        ...
        if single_process:
            ...
            self.tasks = [ProcessTask(task_fn, log_dir)
