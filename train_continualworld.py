...
def ppo_baseline_continualworld(name, args):
    ...
    def task_fn(log_dir): return ContinualWorld(
        name, env_config_path, log_dir, config.seed)
    config.task_fn = lambda: ParallelizedTask(
        task_fn, config.num_workers, log_dir=config.log_dir)

    def eval_task_fn(log_dir): return ContinualWorld(
        name, env_config_path, log_dir, config.seed)
    ...
    config.network_fn = lambda state_dim, action_dim, label_dim: GaussianActorCriticNet_CL(
        ...
        actor_body=FCBody_CL(state_dim + label_dim,
                             hidden_units=(128, 128), gate=torch.tanh),
    ...
    # (i.e., 512 * 2.5, if num_workers is set to 4)
    config.rollout_length = 512 * 10
    config.optimization_epochs = 16
    # with rollout of 5120, 160 mini_batch gives 32 samples per batch
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--seed', help='seed for the experiment', default=8379, type=int)
    ...
    if args.env_name == 'continualworld':
        ...
        raise ValueError(
            '--env_name {0} not implemented'.format(args.env_name))
